...
def tensor2img(batch):
    img = batch.cpu().clone()  # we clone the tensor to not do changes on it
    img = img[:1, :, :, :]
...
def visualization(img_list):
    img_ls = tensor2img(img_list[0])
    img_hr = tensor2img(img_list[1])
    img_gl = tensor2img(img_list[2])
    img_gh = tensor2img(img_list[3])
    ...
    #plt.savefig(current_path + '/samples/images_GAN_alone/{}.jpg'.format(batch))
    plt.show()
