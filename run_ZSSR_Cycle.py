...
import sys
import run_ZSSR_Cycle_single_input
    ...
    for file_ind, input_file in enumerate(files):
        ...
        if gpu == 'all':
            ...
            os.system("xterm -hold -e " + conf.python_path +
                      " %s/run_ZSSR_Cycle_single_input.py '%s' '%s' '%s' '%s' '%s' '%s' alias python &"
        ...
        # The other option is just to run sequentially on a chosen GPU.
            ...
            run_ZSSR_Cycle_single_input.main(input_file, ground_truth_file, kernel_files_str, gpu, conf_name, res_dir)
